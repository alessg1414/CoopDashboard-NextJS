...
interface SubpartidaDialogProps {
  visible: boolean;
  onHide: () => void;
  onSave: () => void;
  mode: "crear" | "editar";
...
export default function SubpartidaDialog({
  visible,
  onHide,
  onSave,
  mode,
  subpartidaData,
}: SubpartidaDialogProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await apiFetch(`${API_BASE}subpartida_contratacion/`, {
      ...
      if (!response.ok) {
        ...
        toast.current?.show({
          severity: "error",
          summary: "Error",
          detail: errorData.error || (mode === "crear" ? "Error al guardar la subpartida" : "Error al actualizar la subpartida"),
          life: 3000,
        });
        return;
      }
      ...
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: mode === "crear" ? "Subpartida creada correctamente" : "Subpartida actualizada correctamente",
        life: 2000,
      });
      ...
      console.error(mode === "crear" ? "Error al guardar la nueva subpartida:" : "Error al actualizar la subpartida:", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  return (
    <>
      <Toast ref={toast} />
      <Dialog
        header={mode === "crear" ? "Añadir subpartida" : "Editar subpartida"}
        visible={visible}
        ...
        footer={
          <div className="flex justify-end gap-2 mt-4">
            ...
            />
            <Button
              label={mode === "crear" ? "Guardar" : "Guardar cambios"}
              icon="pi pi-check"
        ...
        <div className="grid grid-cols-2 gap-6 text-sm mt-2">
          <div>
            ...
            <InputText
              ...
              className={`w-full border rounded-md p-3 bg-white ${
                camposConError.includes("subpartida") ? "border-red-500" : "border-gray-300"
              }`}
          ...
          <div>
            ...
            <InputText
              ...
              className={`w-full border rounded-md p-3 bg-white ${
                camposConError.includes("ano_contrato") ? "border-red-500" : "border-gray-300"
              }`}
              placeholder="Ej: 2025"
            />
          </div>
          ...
          <div className="col-span-2">
            ...
            <InputText
              ...
              className={`w-full border rounded-md p-3 bg-white ${
                camposConError.includes("nombre_subpartida") ? "border-red-500" : "border-gray-300"
              }`}
          ...
          <div>
            ...
            <InputText
              ...
              className={`w-full border rounded-md p-3 bg-white ${
                camposConError.includes("numero_contratacion") ? "border-red-500" : "border-gray-300"
              }`}
          ...
          <div className="col-span-2">
            ...
            <InputNumber
              ...
              locale="es-CR"
              className={`w-full ${
                camposConError.includes("presupuesto_asignado") ? "border-red-500" : ""
              }`}
