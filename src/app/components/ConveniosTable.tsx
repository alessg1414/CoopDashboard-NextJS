...
import ConvenioDialog from "./convenios/ConvenioDialog";
import TimelineModal from "./convenios/TimelineModal";
import EditarRegistroDialog from "./convenios/EditarRegistroDialog";
import { API_BASE, apiFetch } from "@/utils/api";
import { FASES_CONVENIO as fases, SECTORES_CONVENIO, AREAS_ESTRATEGICAS } from "@/utils/constants";
import { Convenio, RegistroProceso } from "@/types";
...
export default function ConveniosTable() {
  ...
  const [seleccionandoParaEditar, setSeleccionandoParaEditar] = useState(false);
  ...
  const opcionesSector = [
    ...
    ...SECTORES_CONVENIO.map(s => s.value === "Otro" ? { label: "Otro", value: "Otro" } : s),
  ];

  // Estado para el filtro de sector
  const [sectorFiltro, setSectorFiltro] = useState<string>("");
  
  // Opciones para Areas Estratégicas (nuevo filtro)
  const opcionesAreaEstrategica = [
    { label: "Todas las áreas", value: "" },
    ...AREAS_ESTRATEGICAS,
  ];

  // Estado para el filtro de Área Estratégica
  const [areaFiltro, setAreaFiltro] = useState<string>("");

  // Mapeo local temporal por convenio (persistido en localStorage) hasta que exista la columna en la DB
  ...
  const fetchData = async () => {
    try {
      ...
      const registrosRes = await apiFetch(`${API_BASE}registro_procesos/`);
      ...
      // Cargar mapeo local de áreas estratégicas (si existe)
      try {
        const stored = localStorage.getItem('area_estrategica_map');
        if (stored) {
          setAreaMap(JSON.parse(stored));
        }
      } catch (e) {
        console.warn('No se pudo cargar area_estrategica_map desde localStorage', e);
      }
  ...
  const updateFaseActual = async (convenioId: number) => {
    try {
      ...
      const response = await apiFetch(`${API_BASE}registro_procesos?convenioId=${convenioId}&latest=true`);
      const lastFase = await response.json();
  
      if (!lastFase || !lastFase.fase_registro) return;
  
      // Actualizar `fase_actual` en la tabla convenios
      await apiFetch(`${API_BASE}convenios/`, {
        method: "PUT",
  ...
  // Obtener el valor del área estratégica para un convenio: prioriza el valor traído por la API si existe,
  // sino usa el mapeo local en localStorage.
  const getAreaForConvenio = (c: any) => {
    return (
      c.area_estrategica ||
      c.areas_estrategicas ||
      c.areaEstrategica ||
      c.areasEstrategicas ||
      areaMap[c.id] ||
      ""
  ...
  // Guardar asignación localmente; si la API ya soporta la columna intentamos actualizar en servidor.
  const handleSetArea = async (convenioId: number, value: string, rowData: any) => {
    // Si la fila ya trae la propiedad desde el servidor, hacer PUT al backend
    const serverHasField = typeof rowData.area_estrategica !== 'undefined' || typeof rowData.areas_estrategicas !== 'undefined' || typeof rowData.areaEstrategica !== 'undefined' || typeof rowData.areasEstrategicas !== 'undefined';
    ...
    if (serverHasField) {
      try {
        await apiFetch(`${API_BASE}convenios/`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: convenioId, area_estrategica: value }),
        });

        setConvenios(prev => prev.map(c => c.id === convenioId ? { ...c, area_estrategica: value } : c));
        toast.current?.show({ severity: 'success', summary: 'Éxito', detail: 'Área estratégica guardada en servidor', life: 2000 });
        return;
      } catch (err) {
        console.warn('Error guardando área en servidor, guardando localmente', err);
      }
    }

    // Guardar localmente
    const nuevo = { ...areaMap, [convenioId]: value };
    setAreaMap(nuevo);
    try {
      localStorage.setItem('area_estrategica_map', JSON.stringify(nuevo));
      toast.current?.show({ severity: 'success', summary: 'Éxito', detail: 'Área estratégica guardada localmente', life: 2000 });
    } catch (e) {
      console.error('Error guardando area_estrategica_map en localStorage', e);
      toast.current?.show({ severity: 'error', summary: 'Error', detail: 'No se pudo guardar localmente', life: 3000 });
    }
  };
  ...
  const handleDeleteConvenio = async () => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}convenios/?id=${selectedConvenioParaEliminar.id}`, {
        method: 'DELETE',
        headers: {
  ...
  const activarSeleccionConvenio = () => {
    ...
    setSelectedConvenioParaEliminar(null);
    setSeleccionandoParaEditar(false);
    setEditingConvenio(null);
  };

  const onSelectionChange = (e:any) => {
    if (seleccionandoConvenio) {
      setSelectedConvenioParaEliminar(e.value);
      setShowConfirmDialog(true);
      return;
    }

    if (seleccionandoParaEditar) {
      setEditingConvenio(e.value);
      setShowDialogConvenio(true);
      setSeleccionandoParaEditar(false);
      return;
    }
  };
  ...
  const handleDeleteRegistro = async (id: number) => {
    try {
      const response = await apiFetch(`${API_BASE}registro_procesos/?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const conveniosFiltrados = convenios.filter(c => {
    ...
    const filtraArea = !areaFiltro || getAreaForConvenio(c) === areaFiltro;

    return filtraBusqueda && filtraFase && filtraSector && filtraArea;
  });
    ...
    <>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Area Estratégica Filter */}
          <Dropdown
            value={areaFiltro}
            options={opcionesAreaEstrategica}
            onChange={e => setAreaFiltro(e.value)}
            optionLabel="label"
            optionValue="value"
            className="p-inputtext-sm center border border-gray-400 rounded-md px-4 py-2 bg-white shadow-sm 
                       focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover:border-blue-500 hover:shadow-lg transition-all duration-300 focus:bg-blue-50"
        ...
        <div className="flex gap-4 mb-4">
          <Button 
            ...
            icon="pi pi-plus" 
            className="bg-[#172951] hover:bg-[#CDA95F] text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-all duration-300 transform hover:scale-105"
            onClick={() => { setEditingConvenio(null); setShowDialogConvenio(true); }}
          />
          <Button
            label="Modificar Convenio"
            icon="pi pi-pencil"
            className="bg-yellow-600 hover:bg-yellow-500 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-all duration-300 transform hover:scale-105"
            onClick={() => { setSeleccionandoParaEditar(!seleccionandoParaEditar); setSeleccionandoConvenio(false); setSelectedConvenioParaEliminar(null); }}
          />
      ...
      <DataTable
        ...
        rows={5}
        filters={filters}
        globalFilterFields={["nombre", "cooperante", "sector", "consecutivo_numerico", "area_estrategica"]}
        responsiveLayout="scroll"
        className={`custom-table ${seleccionandoConvenio || seleccionandoParaEditar ? "cursor-pointer" : ""}`}
        selectionMode={seleccionandoConvenio || seleccionandoParaEditar ? "single" : undefined} 
        onSelectionChange={onSelectionChange} 
      >
        {(seleccionandoConvenio || seleccionandoParaEditar) && <Column selectionMode="single" headerStyle={{ width: "3rem" }} className="p-selection-column"/>}
        ...
        <Column
          ...
          body={(row: any) => (
            <div className="w-60">
              <Dropdown
                value={getAreaForConvenio(row)}
                options={opcionesAreaEstrategica}
                onChange={(e) => handleSetArea(row.id, e.value, row)}
                optionLabel="label"
                optionValue="value"
      ...
      <ConvenioDialog
        ...
        onHide={() => { setShowDialogConvenio(false); setEditingConvenio(null); }}
        onRefresh={fetchData}
