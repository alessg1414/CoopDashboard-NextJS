...
import ObservacionesViajes from "./viajes/ObservacionesViajes";
...
export default function AcuerdosViajes() {
  ...
  const fetchViajes = async () => {
    try {
      const response = await apiFetch(`${API_BASE}viajes_al_exterior/`);
  ...
  const handleDeleteViaje = async (id: number) => {
    try {
      const response = await apiFetch(`${API_BASE}viajes_al_exterior/?id=${id}`, {
        method: 'DELETE',
      });
