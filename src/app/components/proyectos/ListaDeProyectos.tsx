...
import { Proyecto, Area } from "@/types";
...
export default function Proyectos() {
  ...
  const handleFileUpload = async (event: any, rowData: Proyecto) => {
    ...
    await apiFetch(`${API_BASE}proyectos/`, {
      method: "PUT",
  ...
  const handleDeleteProyecto = async (NumProyecto?: number) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}proyectos/?NumProyecto=${NumProyecto}`, { method: "DELETE" });
