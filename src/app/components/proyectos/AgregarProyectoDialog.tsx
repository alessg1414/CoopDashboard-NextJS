...
import {
  ETAPAS_PROYECTO as etapas,
  ENTIDADES_DEPENDENCIAS as EntidadesDependencias,
  MODALIDADES_PROYECTO as modalidades,
  SECTORES_PROYECTO as sectores,
  TIPO_COOPERACIONES as TipoCooperaciones,
  AUTORIDADES_A_CARGO as AutoridadesAcargo,
  DEPENDENCIAS_CANTIDAD as dependencias,
...
export default function AgregarProyectoDialog({ visible, onHide, onSave }: AgregarProyectoDialogProps) {
 ...
 useEffect(() => {
  ...
  /*Declaracion de los elementos de cada Lista en Ingreso de Proyecto — importados de constants.ts*/
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (agregandoArea && nuevaArea.trim() !== "") {
        const resArea = await apiFetch(`${API_BASE}areas/`, {
          method: "POST",
      ...
      const response = await apiFetch(`${API_BASE}proyectos/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
