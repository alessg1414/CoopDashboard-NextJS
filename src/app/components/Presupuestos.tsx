...
import { SolicitudPresupuesto } from "@/types";
import AgregarOrdenDialog from "./presupuestos/AgregarOrdenDialog";
import EditarOrdenDialog from "./EditarOrdenDialog";
import SubpartidaDialog from "./presupuestos/SubpartidaDialog";
...
export default function PresupuestosTable() {
  ...
  const fetchFichaYSolicitudes = async (subpartida: string, ano: string) => {
    try {
      ...
      const response = await apiFetch(`${API_BASE}solicitud_presupuesto/`);
      const data = await response.json();
          ...
          <div className="p-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button label="Eliminar" icon="pi pi-trash" className="p-button-danger" onClick={async () => {
                ...
                try {
                  const response = await apiFetch(`${API_BASE}solicitud_presupuesto/?id=${ordenAEliminar.id}`, { method: "DELETE" });
          ...
          <div className="p-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button label="Eliminar" icon="pi pi-trash" className="p-button-danger" onClick={async () => {
                ...
                try {
                  const response = await apiFetch(`${API_BASE}subpartida_contratacion/?id=${fichaSeleccionada.id}`, { method: "DELETE" });
